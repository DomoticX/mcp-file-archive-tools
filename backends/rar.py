"""RAR (.rar) backend: Rar.exe (read/write) and UnRAR.exe (read-only) in bin/winrar/."""
...
def list_archive(archive_path: str, password: str | None = None) -> dict[str, Any]:
    ...
    _check(rc, out, err, "Listing archive", codes=RAR_EXIT_CODES)
...
def extract_archive(
    ...
    rc, out, err = _run(UNRAR_EXE, args)
    _check(rc, out, err, "Extracting archive", codes=RAR_EXIT_CODES)
...
def add_to_archive(
    ...
    archive = _normalize_archive_path(archive_path, default_suffix=".rar")
    ...
    _check(rc, out, err, "Adding files to archive", codes=RAR_EXIT_CODES)
...
def update_archive(
    ...
    rc, out, err = _run(RAR_EXE, args, cwd=cwd)
    _check(rc, out, err, "Updating archive", codes=RAR_EXIT_CODES)
...
def move_to_archive(
    ...
    rc, out, err = _run(RAR_EXE, args, cwd=cwd)
    _check(rc, out, err, "Moving files into archive", codes=RAR_EXIT_CODES)
...
def delete_from_archive(archive_path: str, files: list[str]) -> dict[str, Any]:
    ...
    _check(rc, out, err, "Deleting from archive", codes=RAR_EXIT_CODES)
...
def rename_in_archive(archive_path: str, renames: dict[str, str]) -> dict[str, Any]:
    ...
    for old, new in renames.items():
        args.extend([old, new])
    rc, out, err = _run(RAR_EXE, args)
    _check(rc, out, err, "Renaming archive entries", codes=RAR_EXIT_CODES)
...
def test_archive(archive_path: str, password: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "ok": rc == 0,
        "exit_code": rc,
        "message": _describe_exit_code(rc, RAR_EXIT_CODES),
...
def repair_archive(archive_path: str) -> dict[str, Any]:
    ...
    _check(rc, out, err, "Repairing archive", codes=RAR_EXIT_CODES)
...
def lock_archive(archive_path: str) -> dict[str, Any]:
    ...
    rc, out, err = _run(RAR_EXE, args)
    _check(rc, out, err, "Locking archive", codes=RAR_EXIT_CODES)
...
def set_archive_comment(archive_path: str, comment: str) -> dict[str, Any]:
    ...
    try:
        ...
        if comment_file.exists():
            comment_file.unlink()
    _check(rc, out, err, "Setting archive comment", codes=RAR_EXIT_CODES)
...
def get_archive_comment(archive_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        if comment_file.exists():
            comment_file.unlink()
    _check(rc, out, err, "Reading archive comment", codes=RAR_EXIT_CODES)
...
def convert_to_sfx(archive_path: str, sfx_name: str | None = None) -> dict[str, Any]:
    ...
    rc, out, err = _run(RAR_EXE, args, cwd=archive.parent)
    _check(rc, out, err, "Converting archive to SFX", codes=RAR_EXIT_CODES)
...
def search_archive(archive_path: str, search_string: str, password: str | None = None) -> dict[str, Any]:
    ...
    rc, out, err = _run(RAR_EXE, args)
    _check(rc, out, err, "Searching archive", codes=RAR_EXIT_CODES)
...
def print_file_from_archive(
    ...
    if rc != 0:
        raise RuntimeError(
            f"Printing file failed: {_describe_exit_code(rc, RAR_EXIT_CODES)} (exit {rc}).\n{_decode(err_bytes)}"
        )
    try:
