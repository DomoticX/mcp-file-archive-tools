...
# Only ".rar" and ".arj" are wired up to an implementation today.
...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "(.rar, list_archive/extract_archive/... tools) and ARJ archives "
        "(.arj, arj_* tools) are fully supported: list, extract, create/add, "
        "update, delete, rename, test, comment, convert-to-SFX, search and "
        "print-file, plus format-specific extras (RAR lock/repair, ARJ "
        "garble/recover). Other formats are on the roadmap; call "
...
def extract_any_archive(
    ...
    Currently .rar and .arj are implemented; other known extensions raise a
